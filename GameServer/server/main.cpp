...
#pragma endregion
    ...
    // search the session list for an existing session that is not full and put the player in it
    for (auto session : ALL_SESSIONS)
    {
        if (session->player1 == nullptr)
        {
            // if player1 is not set, set the player as player1
            ...
            DEBUG_PRINT("Player added to existing session as player1\n");
            cout << INFO << "Player added to existing session as player1" << RESET << endl;
    ...
    // if a session is full, a detached thread will be created to handle the gameplay
...
#pragma region Ask for Game Mode
    ...
    DEBUG_PRINT("------------------ Asking client if they want to play against player or ai -----------------\n");
...
#pragma endregion
    ...
    if (player->get_chosen_game() == 0 && player->get_chosen_game_mode() == 1)
    {
        ...
        // If AI, start MCTS checkers game
    ...
    {
        // If multiplayer, start multiplayer checkers game (still needs implementation)
        DEBUG_PRINT("Starting multiplayer checkers game\n");
        cout << INFO << "Starting multiplayer checkers game" << RESET << endl;
        // Here you would start the multiplayer checkers game logic
        // For now, we will just simulate the game with a sleep
        this_thread::sleep_for(chrono::seconds(5)); // simulate the game for 5 seconds
...
#pragma region Ask which player/send which player they are
    //     // Ask client which player they want to be
    //     string question = "Who do you want to be? [1/2] or [q] to quit: ";
    ...
    //     }
    //     string player_input;
    //     string *ptr_player_input = &player_input;
    //     get_response(player_socket, ptr_player_input);

    //     if (player_input != "1" && player_input != "2")
    //     {
    //         send_to(player->get_socket(), "TEXT", "Invalid response. Exiting.");
    ...
    //     {
    //         send_to(player->get_socket(), "TEXT", "Exiting game.");
    //         send_to(player->get_socket(), "GOODBYE");
    //         close(player->get_socket());
    //         return save_and_exit(mcts_tree);
    //     }
    ...
    //         return save_and_exit(mcts_tree);
    //     }
    //     int player_id = stoi(player_input); // this holds which player the client wants to be, either 1 or 2
    ptr_session->player1->set_id(1); // set player 1 id
    // DEBUG_PRINT("Player 1 id set to " + to_string(ptr_session->player1->get_id()) + "\n");

    if (ptr_session->player2 != nullptr)
    {
        ptr_session->player2->set_id(2); // set player 2 id
        // DEBUG_PRINT("Player 2 id set to " + to_string(ptr_session->player2->get_id()) + "\n");
    }
    ...
    if (ptr_session->player2 != nullptr)
    {
        // if there is a second player, send them the same message
        string player2_id_str = "You are player " + to_string(ptr_session->player2->get_id()) + ".";
        send_to(ptr_session->player2->get_socket(), "TEXT", player2_id_str);
    }
#pragma endregion
